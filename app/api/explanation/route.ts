...
function buildFallback(body: any) {
...
export async function POST(req: Request) {
  if (!rateLimit(`explanation:${getClientIp(req)}`, 40, 60000)) {
  ...
  // Never construct the SDK with a placeholder secret. If the API key is
  // missing, serve the localized rule-based reasoning engine instead.
  if (!process.env.GROQ_API_KEY) {
    console.warn('[explanation] Missing GROQ_API_KEY — using local reasoning fallback.');
